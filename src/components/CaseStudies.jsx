...
function caseStudies() {
  ...
  return (
    ...
    <section id="case-studies" className="min-h-auto ml-3 flex items-center">
      <div className="container mx-auto font-family-arial mt-10 sm:mt-20 text-left">
        <h1 style={{ color: '#461E7D' }} className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold">Case Studies</h1>
  ...
  {projectsData.map((project) => (
    <div style={{ backgroundColor: '#F0F0F0' }} key={project.id} className="mt-6 bg-white transform transition-transform hover:scale-105">
        ...
        <div className="flex flex-col items-left p-5">
          ...
          <img alt={project.title} className="mt-4 w-100 h-40 object-cover" src={project.imgSrc} />
          <h2 className="text-xs sm:text-xs md:text-md lg:text-sm font-light text-black mt-2">{project.overview}</h2>
          <Link to={`/${project.id}`}>
